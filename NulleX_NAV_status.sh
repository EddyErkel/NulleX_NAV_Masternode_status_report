...
f_verify_masternode_status ()
{
    ...
    $nullexcli masternode status | grep message | sed -e "s/^  \"message\": //"
    ...
    if [ "$masternodestatus" == "4" ]; then
            f_dispnorm "NulleX masternode status is OK."
    else
            f_disperr "NulleX masternode status is NOT OK!"
    fi
...
f_verify_balance ()
{
    ...
    # balancechange=1                                     # Unhash for script testing          
        ...
        if [ $newbalance -gt $oldbalance ]; then
            f_dispnorm "NulleX rewards have been added since last check."
            ...
            if [ "$balancechange" -eq "1" ]; then
                ...
                f_dispnorm "NulleX balance unchanged since check at $datebalance."
            fi
        fi
    else
        f_dispnorm "Current balance : $newbalance"
